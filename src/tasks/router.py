...
from src.tasks.dtos import TaskShema,TaskResponseSchema
...
def create_task(body:TaskShema,db:Session=Depends(get_db),user:UserModel=Depends(is_authenticated)):
...
def update_task(body:TaskShema,task_id:int,db:Session=Depends(get_db),user:UserModel=Depends(is_authenticated)):
